...
class Assignment3:
    def __init__(self):
        ...
        self.ass2 = assignment2.Assignment2()
        pass
    ...
    def integrate(self, f: callable, a: float, b: float, n: int ) -> np.float32:
        ...
        if n >= 3:
            ...
            delta = np.float32((b-a)/(n-1))
            # calculated by simpsons rule
            S = delta / 3. * np.sum(y[0:-1:2]+ 4. * y[1::2] + y[2::2])
            # for a case of reverse (b is larger than a) the area is negative.
            if flag_rev:
                S = -1*S
            return np.float32(S)
            ...
            return  np.float32((b-a)*0.5*(f(a) + f(b)))
        elif n == 1:
            return np.float32((b-a) * f(0.5*(b+a)))
        else:
            return np.float32(0)
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        result = np.float32(0)
        inter_points = self.ass2.intersections(f1,f2,1,100)

        if len(inter_points) < 2:
            return np.NaN


        for i in np.arange(len(inter_points)-1):
            result+= np.abs(self.integrate(func,np.float32(inter_points[i]),np.float32(inter_points[i+1]),100))
