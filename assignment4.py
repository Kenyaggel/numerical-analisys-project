...
class Assignment4A:
    ...
    def fit(self, f: callable, a: float, b: float, d:int, maxtime: float) -> callable:
        ...
        """
        Ti = time.time()
        #estimated time for computing is about 35000000 cells per sec as there are d*pnum cells in the matrix
        #so p_num should be
        if d != 0:

            p_num  = int((5000 * maxtime) /d)
        else:
            p_num = 10000
        x = np.linspace(a,b,p_num)
        ...
        for x_s in x:
            ls = []
            for i in np.arange(d,-1,-1):
                ls.append(np.power(x_s,i))
